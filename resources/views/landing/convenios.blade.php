    ...
    <section id="convenios" class="pb-24 bg-pg-negro">
        <div class="max-w-[1520px] mx-auto px-4 sm:px-6 lg:px-8">
            @foreach($conConvenio as $p)
                ...
                <div class="animate-on-scroll mx-auto w-fit flex flex-wrap items-baseline justify-center gap-x-3 gap-y-1 bg-pg-carbon border border-pg-rojo/30 rounded-2xl px-6 py-4 mb-4 text-center">
            ...
            @forelse($convenios as $grupo)
                <div class="mt-16">
                    <h2 class="animate-on-scroll font-display text-3xl uppercase tracking-wide text-pg-tiza mb-8">{{ $grupo['titulo'] }}</h2>
                        ...
                        @foreach($grupo['convenios'] as $i => $c)
                            <div class="animate-on-scroll" style="animation-delay: {{ $i * 0.1 }}s">
                                ...
                                <div class="group flotar" style="animation-delay: -{{ ($i % 4) * 1.5 }}s">
                                    <div class="h-36 sm:h-44 bg-white rounded-2xl p-5 sm:p-6 flex items-center justify-center shadow-xl shadow-black/40 ring-1 ring-white/10 transition-all duration-300 group-hover:scale-105 group-hover:ring-4 group-hover:ring-pg-rojo/60">
                                        @if($c['logo'])
                                            <img src="{{ $c['logo'] }}" alt="{{ $c['nombre'] }}" loading="lazy" class="max-h-full max-w-full object-contain">
                                        @else
                                            <span class="text-gray-800 font-semibold font-modern text-lg sm:text-xl text-center leading-tight">{{ $c['nombre'] }}</span>
                                        @endif
                                    </div>
            ...
            @empty
                <p class="text-center text-pg-tiza/60 font-modern py-16">Todavía no publicamos convenios. Pregunta en el mesón si tu empresa o institución tiene uno.</p>
            @endforelse
