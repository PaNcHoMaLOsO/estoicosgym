    ...
    @if(count($fotos))
        <section id="fotos" class="py-24 bg-pg-carbon">
            <div class="max-w-[1520px] mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {{-- Todas del mismo tamaño: la primera ocupaba cuatro celdas y con
                     cuatro fotos quedaba un hueco abajo. --}}
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    @foreach($fotos as $i => $foto)
                        <figure class="animate-on-scroll group relative aspect-[4/5] overflow-hidden rounded-2xl bg-pg-negro" style="animation-delay: {{ ($i % 4) * 0.1 }}s">
