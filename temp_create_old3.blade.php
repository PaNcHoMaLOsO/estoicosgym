...
@section('content')
    @if ($errors->any())
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <i class="fas fa-exclamation-circle"></i> <strong>┬íError!</strong>
            <ul class="mb-0 mt-2">
    ...
    <form action="{{ route('admin.pagos.store') }}" method="POST" id="formPago">
                    ...
                    <select class="form-control form-control-lg select2 @error('id_inscripcion') is-invalid @enderror" 
                        ...
                        @foreach($inscripciones as $insc)
                            ...
                            <option value="{{ $insc->id }}" 
                                ...
                                ­ƒæñ {{ $insc->cliente->nombres }} {{ $insc->cliente->apellido_paterno }} ({{ $insc->membresia->nombre }})
                            </option>
                        @endforeach
                    </select>
        ...
        <!-- 2. INFORMACI├ôN DEL CLIENTE E HISTORIAL -->
            ...
            <div class="card-body">
                ...
                <div id="seccion-completo" class="pago-section d-none">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="font-weight-bold">
                                    Monto a Pagar (Calculado Autom├íticamente)
                                </label>
                                <div class="input-group input-group-lg">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">$</span>
                                ...
                                <small class="text-muted d-block mt-2">Ô£ô Se pagar├í autom├íticamente el saldo exacto pendiente</small>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                    ...
                    <div class="resumen-pago alert alert-success" id="resumen-completo">
                        <strong>Ô£ô Resultado:</strong> El cliente quedar├í PAGADO COMPLETAMENTE
                    </div>
                </div>
                ...
                <div id="seccion-mixto" class="pago-section d-none">
                        ...
                        <div class="metodo-box">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control form-control-lg monto-mixto" id="monto_metodo2" 
                                       step="1000" min="0" placeholder="0">
                                <small class="text-muted d-block mt-2">Ô£ô M├®todo: Efectivo</small>
                            </div>
                        </div>
                    </div>
                    <div class="resumen-pago alert alert-warning" id="resumen-mixto">
                        ...
                        <span id="estado-mixto" class="ml-3" style="color: #dc3545;">ÔØî Monto incompleto</span>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('­ƒÜÇ Script iniciado - DOMContentLoaded');
    ...
    // Validaci├│n inicial
    console.log('Ô£ô selectInscripcion:', selectInscripcion?.id);
    console.log('Ô£ô clienteInfoSection:', clienteInfoSection?.id);
    console.log('Ô£ô tipoPagoSection:', tipoPagoSection?.id);
    console.log('Ô£ô datosPagoSection:', datosPagoSection?.id);
    console.log('Ô£ô formPago:', formPago?.id);
    console.log('Ô£ô btnSubmit:', btnSubmit?.id);

    // Funci├│n para RESETEAR todos los campos
    function resetearFormulario() {
        console.log('­ƒöä Reseteando formulario...');
        try {
            const elementos = [
            ...
            elementos.forEach(id => {
                ...
                if (el) {
                    ...
                    console.log(`  Ô£ô Limpiado: ${id}`);
                } else {
                    console.warn(`  ÔÜá´©Å Elemento no encontrado: ${id}`);
                }
            });
            ...
            if (fechaPagoEl) {
                ...
                console.log(`  Ô£ô Fecha establecida: ${fechaHoy}`);
            }
            ...
            if (radioAbono) {
                ...
                console.log('  Ô£ô Tipo pago: abono');
            }
            ...
            console.log('  Ô£ô Secciones reseteadas');
            ...
            elementos_resumen.forEach(id => {
                ...
                if (el) {
                    if (id === 'estado-mixto') {
                        el.innerHTML = 'ÔØî Monto incompleto';
                    } else {
                        el.textContent = '0';
                    }
                }
            ...
            console.log('Ô£ô Formulario reseteado exitosamente - updateSubmitButton ser├í llamado');
            // NO deshabilitamos el bot├│n aqu├¡ - ser├í actualizado despu├®s por mostrarCliente
        } catch(error) {
            console.error('ÔØî Error al resetear formulario:', error);
    ...
    function cargarHistorial(inscripcionId) {
        console.log(`­ƒôï Cargando historial para inscripci├│n: ${inscripcionId}`);
        ...
        fetch(url)
            ...
            .then(data => {
                console.log('Ô£ô Historial cargado:', data);
                ...
                console.log(`Ô£ô Historial renderizado: ${data.pagos.length} pagos`);
            })
            .catch(error => {
                console.error('ÔØî Error cargando historial:', error);
    ...
    function mostrarCliente() {
        console.log('­ƒæñ mostrarCliente() invocado');
        ...
        if (value) {
            ...
            if (!option) {
                console.error('ÔØî No se encontr├│ el option element');
                return;
            }
            ...
            try {
                ...
                const elementos_cliente = {
                    ...
                    'diasRestantes': dias > 0 ? dias + ' d├¡as' : '­ƒö┤ Vencido'
                };
                ...
                if (elMontoCompleto) {
                    ...
                    console.log(`  Ô£ô monto_completo: $${pendiente.toLocaleString('es-CO')}`);
                } else {
                    console.warn('  ÔÜá´©Å monto_completo no encontrado');
                }

                if (elMontoAbonComplet) {
                    elMontoAbonComplet.value = pendiente;
                    console.log(`  Ô£ô monto_abonado_completo: ${pendiente}`);
                } else {
                    console.warn('  ÔÜá´©Å monto_abonado_completo no encontrado');
                }

                if (elTargetMixto) {
                    elTargetMixto.textContent = pendiente.toLocaleString('es-CO');
                    console.log(`  Ô£ô target-mixto: ${pendiente.toLocaleString('es-CO')}`);
                } else {
                    console.warn('  ÔÜá´©Å target-mixto no encontrado');
                }
                ...
                console.log('  Ô£ô Secciones visibles');
                ...
                // ACTUALIZAR ESTADO DEL BOT├ôN (importante despu├®s de resetear)
                updateSubmitButton();
                
                console.log('Ô£ô mostrarCliente() completado exitosamente');
            } catch(error) {
                console.error('ÔØî Error en mostrarCliente():', error);
            }
        } else {
    ...
    console.log('ÔÜÖ´©Å Inicializando Select2...');
    $('#id_inscripcion').select2({
        width: '100%',
        language: 'es',
        placeholder: '­ƒöì Buscar por nombre, RUT o email',
        allowClear: true,
    ...
    console.log('Ô£ô Select2 inicializado');

    // Eventos Select2
    console.log('­ƒôî Agregando event listeners...');
    $('#id_inscripcion').on('select2:select', function(e) {
        console.log('­ƒôî Evento select2:select disparado:', e.params.data);
        mostrarCliente();
    });
    
    $('#id_inscripcion').on('select2:clear', function() {
        console.log('­ƒôî Evento select2:clear disparado');
        clienteInfoSection.classList.add('d-none');
        tipoPagoSection.classList.add('d-none');
        datosPagoSection.classList.add('d-none');
        btnSubmit.disabled = true;
    });
    console.log('Ô£ô Event listeners agregados');

    // Cambio de tipo de pago
    document.querySelectorAll('input[name="tipo_pago"]').forEach(radio => {
        radio.addEventListener('change', function() {
            console.log(`­ƒöÿ Tipo de pago cambiado a: ${this.value}`);
            document.querySelectorAll('.pago-section').forEach(s => s.classList.add('d-none'));
            document.querySelectorAll('.tipo-pago-card').forEach(c => c.classList.remove('active'));
            
            if (this.value === 'abono') {
                console.log('  ÔåÆ Mostrando secci├│n ABONO');
                ...
                console.log('  ÔåÆ Mostrando secci├│n COMPLETO');
                ...
                console.log('  ÔåÆ Mostrando secci├│n MIXTO');
    ...
    const validarMixto = function() {
        ...
        if (total === target) {
            estado.innerHTML = 'Ô£ô Monto correcto';
            ...
            estado.innerHTML = 'ÔØî Monto excede';
            ...
            estado.innerHTML = 'ÔØî Monto incompleto';
    ...
    function updateSubmitButton() {
        try {
            ...
            if (!tipoPago) {
                console.log('ÔÜá´©Å updateSubmitButton: Sin tipo de pago seleccionado');
                btnSubmit.disabled = true;
                return;
            }

            console.log(`­ƒöÿ updateSubmitButton: Tipo = ${tipoPago.value}`);
            ...
            if (tipoPago.value === 'abono') {
                ...
                if (!elMonto || !elTotal || !elMetodo) {
                    console.warn('ÔÜá´©Å Elementos faltantes en abono:', {elMonto, elTotal, elMetodo});
                    btnSubmit.disabled = true;
                    return;
                }
                ...
                if (!elMetodo) {
                    console.warn('ÔÜá´©Å Elemento faltante: id_metodo_pago_completo');
                    btnSubmit.disabled = true;
                    return;
                }
                const metodo = elMetodo.value;
                ...
                if (!el1 || !el2 || !elTarget || !elMetodo) {
                    console.warn('ÔÜá´©Å Elementos faltantes en mixto:', {el1, el2, elTarget, elMetodo});
                    btnSubmit.disabled = true;
                    return;
                }
            ...
            console.error('ÔØî Error en updateSubmitButton():', error);
            btnSubmit.disabled = true;
        }
    }
    ...
    formPago.addEventListener('submit', function(e) {
        console.log('­ƒôñ Formulario siendo enviado');
        try {
            ...
            console.log('Ô£ô Valores de formulario populados');
        } catch(error) {
            console.error('ÔØî Error al enviar formulario:', error);
            e.preventDefault();
        }
    });
    
    console.log('­ƒÄë Script completamente inicializado');
});
</script>
@endsection
